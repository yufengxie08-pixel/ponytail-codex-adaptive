...
Codex uses an adaptive, quality-first adapter: SessionStart injects a small core, then each prompt is routed to a `game`, `visual`, `engineering`, or conservative `quality` profile. A project can override automatic routing with `.ponytail.json` or `.codex/ponytail.json`:

```json
{ "profile": "game" }
```

See [docs/codex-adaptive.md](docs/codex-adaptive.md) for the project and global configuration details. Claude Code and the other adapters retain the original full Ponytail ruleset.
