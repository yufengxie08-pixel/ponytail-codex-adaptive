...
const { clearMode, isCodex, isQoder, readMode, setMode, writeHookOutput } = require('./ponytail-runtime');
const { getCodexPonytailInstructions, getPonytailInstructions } = require('./ponytail-instructions');
...
function finish() {
  ...
  try {
    ...
    // Codex gets the compact core at SessionStart and a routed profile on each
    // prompt. Reclassifying per turn lets a thread move from implementation to
    // visual or game work without repeating the full ruleset in every context.
    if (isCodex && !deactivated) {
      const currentMode = readMode() || getDefaultMode();
      if (currentMode && currentMode !== 'off') {
        const header = modeSwitched
          ? 'PONYTAIL MODE CHANGED — level: ' + currentMode + '\n\n'
          : isReportOnly
            ? 'PONYTAIL MODE ACTIVE — level: ' + currentMode + '\n\n'
            : '';
        writeHookOutput(
          'UserPromptSubmit',
          currentMode,
          header + getCodexPonytailInstructions(currentMode, {
