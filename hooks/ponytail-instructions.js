...
const { readProfile, resolveProfile } = require('./ponytail-profile');
...
function getCodexPonytailInstructions(mode, options = {}) {
  const configuredMode = normalizePersistedMode(mode) || DEFAULT_MODE;
  if (configuredMode === 'off') return '';
  if (INDEPENDENT_MODES.has(configuredMode)) {
    return 'PONYTAIL CODEX PROFILE: REVIEW\n\n' + readProfile('engineering');
  }

  const profile = options.profile === 'core'
    ? 'core'
    : resolveProfile({ cwd: options.cwd, prompt: options.prompt, profile: options.profile });
  const modeNote = configuredMode === 'lite'
    ? 'Apply the profile selectively; do not add infrastructure for hypothetical needs.'
    : configuredMode === 'ultra'
      ? 'Apply the profile on every response and review the diff for accidental simplification before finishing.'
      : 'Apply the profile on every response while preserving all requested quality and behavior.';

  const profileRules = profile === 'core' ? '' : '\n\n' + readProfile(profile);
  return 'PONYTAIL MODE ACTIVE — level: ' + configuredMode + '\n' +
    'PONYTAIL CODEX PROFILE: ' + profile.toUpperCase() + ' | intensity: ' + configuredMode + '\n\n' +
    readProfile('core') + profileRules + '\n\n' + modeNote;
}

module.exports = {
  filterSkillBodyForMode,
  getCodexPonytailInstructions,
